...
public class ComputeEngineInstance extends AbstractCloudSlave {
    ...
    private void _terminateThreadedWork(Operation stopResponse) {
...

        {
            ...
            try {
                LOGGER.log(
                        ...
                        new Object[] { name });
                Operation stopResponseFinal = cloud
                        .getClient()
        ...
        // and the instance configuration allows persisting the same or more than the
        // current number of instances, choose to persist the current instance
        ...
        if (instanceConfiguration != null) {
            ...
            Map<InstanceConfiguration, Integer> projectedInstanceCountPerConfig = cloud
        ...
        if (!persist) {
            ...
            try {
                LOGGER.log(
                        ...
                        new Object[] { name });
                Operation terminateResponseFinal = cloud
                        .getClient()
    ...
    protected void _terminate(TaskListener listener) throws IOException, InterruptedException {
        try {
            ...
            // If the instance exists in the cloud, attempt to terminate it.
            if (instanceExistsInCloud) {
                LOGGER.log(Level.INFO, "Stopping instance {0}", new Object[] {name});

                Operation stopResponse =
                    cloud.getClientV2().stopInstance(cloud.getProjectId(), nameFromSelfLink(zone), name);
