...
public class InstanceConfiguration implements Describable<InstanceConfiguration> {
  ...
  public ComputeEngineInstance provision(Instance instance)
    ...
    try {
      ...
      if (instance == null) {
        ...
        // The VM is being reprovisioned; update its SSH public key if necessary
        if (windowsConfiguration == null) {
          log.fine("Updating SSH key for [" + instance.getName() + "]");
          updateSSHPublicKeyOnVM(instance, runAsUser);
        }
  ...
  // If the given instance is using a key pair, then make sure the public key on
  //   the instance matches what the plugin expects
  // This ensures that the public SSH key gets updated when reusing a VM
  //
  // This will remove any SSH keys for other users than the plugin's user

  private void updateSSHPublicKeyOnVM(Instance instance, String sshUser)
      throws IOException, InterruptedException, OperationException {
    if (sshKeyCredential instanceof GoogleKeyPair) {
      Metadata.Items items =
          new Metadata.Items()
              .setKey(SSH_METADATA_KEY)
              .setValue(((GoogleKeyPair) sshKeyCredential).getPublicKey());
