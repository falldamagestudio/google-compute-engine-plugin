...
public class InstanceConfiguration implements Describable<InstanceConfiguration> {
    ...
    public ComputeEngineInstance provision(Instance instance)
        ...
        try {
            GoogleKeyCredential sshKeyCredential = null;
            ...
            if (instance == null) {
                ...
                if (windowsConfiguration == null) {
                    if (sshConfiguration != null) {
                        log.info("User selected to use a custom ssh private key for [" + instance.getName() + "]");
                        sshKeyCredential =
                                configureSSHPrivateKey(sshConfiguration.getCustomPrivateKeyCredentialsId(), runAsUser);
                    } else {
                        log.info("User selected to use an autogenerated ssh key pair for [" + instance.getName() + "]");
                ...
                operation = cloud.getClient2()
                        .startInstance(cloud.getProjectId(), nameFromSelfLink(instance.getZone()), instance.getName());
