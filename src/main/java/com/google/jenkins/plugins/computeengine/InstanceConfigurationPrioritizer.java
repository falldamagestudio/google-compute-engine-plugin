...
public class InstanceConfigurationPrioritizer {
    ...
    /**
     * Choose config from list of available configs. Current implementation use
     * round robin strategy
     * starting at semi random element of list. Because most of times arriving
     * requests asks for only
    ...
    /**
     * Choose instance from list of available instances. Current implementation use
     * round robin
    ...
    // return those configs that have at least one provisionable instance associated
    // with it
    ...
    List<InstanceConfiguration> getConfigsWithProvisionableInstances(
        ...
        List<InstanceConfiguration> configsWithProvisionableInstances = configs.stream()
                .filter(
                        config -> filterInstancesForConfig(config, provisionableInstances.stream())
                                .findAny()
                                .isPresent())
    ...
    // return those provisionable instances that are associated with that particular
    // config

    List<Instance> getProvisionableInstancesForConfig(
            InstanceConfiguration config, List<Instance> provisionableInstances) {
        List<Instance> provisionableInstancesForConfig = filterInstancesForConfig(config,
                provisionableInstances.stream())
                .collect(Collectors.toList());
        return provisionableInstancesForConfig;
    }

    // Given a list of configs, and a list of projected instance count for those
    // configs
    ...
    List<InstanceConfiguration> getConfigsWithSpareCapacity(
        ...
        List<InstanceConfiguration> configsWithSpareCapacity = configs.stream()
                .filter(
                        config -> config.getMaxNumInstancesToCreate() > projectedInstanceCountPerConfig
    ...
    // Given a list of relevant configs, a list of all instances related to these
    // configs,
    ...
    // - a ConfigAndInstance identifying a config & an instance - use this config,
    // re-use this
    // instance
    // - a ConfigAndInstance identifying a config, but no instance - use this
    // config, provision a new
    // instance
    ...
    public ConfigAndInstance getConfigAndInstance(
        ...
        if (!provisionableInstances.isEmpty()) {
            List<InstanceConfiguration> configsWithProvisionableInstances = getConfigsWithProvisionableInstances(
                    configs, provisionableInstances);
            if (!configsWithProvisionableInstances.isEmpty()) {
                InstanceConfiguration tentativeConfig = chooseConfigFromList(configsWithProvisionableInstances);
                List<Instance> provisionableInstancesForConfig = getProvisionableInstancesForConfig(tentativeConfig,
                        provisionableInstances);
        ...
        // Second, look for a config that has no provisionable instances but available
        // capacity
        // If we find one, choose that config, but leave the instance blank

        List<InstanceConfiguration> configsWithSpareCapacity = getConfigsWithSpareCapacity(configs,
                projectedInstanceCountPerConfig);
    ...
    Set<String> getProjectedInstanceNamesForConfig(
        ...
        Stream<Instance> currentInstancesForConfig = filterInstancesForConfig(config, allInstances.stream());
        Set<String> currentInstanceNamesForConfig = currentInstancesForConfig.map(instance -> instance.getName())
                .collect(Collectors.toSet());

        Stream<InstanceOperationTracker.InstanceOperation> insertsInProgressForConfig = filterInstanceOperationsForConfig(
                config, insertsInProgress.stream());
        Set<String> insertNamesInProgressForConfig = insertsInProgressForConfig
                .map(instanceOperation -> instanceOperation.getName())
                .collect(Collectors.toSet());

        Stream<InstanceOperationTracker.InstanceOperation> deletesInProgressForConfig = filterInstanceOperationsForConfig(
                config, deletesInProgress.stream());
        Set<String> deleteNamesInProgressForConfig = deletesInProgressForConfig
                .map(instanceOperation -> instanceOperation.getName())
                .collect(Collectors.toSet());

        Set<String> projectedInstanceNamesForConfig = new HashSet<String>(currentInstanceNamesForConfig);
    ...
    public Map<InstanceConfiguration, Integer> getProjectedInstanceCountPerConfig(
        ...
        Map<InstanceConfiguration, Integer> instancesPerConfig = new HashMap<InstanceConfiguration, Integer>();
