...
public class InstanceConfigurationPrioritizer {
    ...
    Stream<Instance> filterInstancesForConfig(InstanceConfiguration config, Stream<Instance> instances) {
        return instances.filter(instance -> instance.getLabels()
    ...
    List<InstanceConfiguration> getConfigsWithProvisionableInstances(
        ...
        List<InstanceConfiguration> configsWithProvisionableInstances = configs.stream()
                .filter(config -> filterInstancesForConfig(config, provisionableInstances.stream())
                        .findAny()
                        .isPresent())
    ...
    List<Instance> getProvisionableInstancesForConfig(
        ...
        List<Instance> provisionableInstancesForConfig = filterInstancesForConfig(
                        config, provisionableInstances.stream())
                .collect(Collectors.toList());
    ...
    List<InstanceConfiguration> getConfigsWithSpareCapacity(
            List<InstanceConfiguration> configs, Map<InstanceConfiguration, Integer> projectedInstanceCountPerConfig) {

        List<InstanceConfiguration> configsWithSpareCapacity = configs.stream()
                .filter(config ->
                        config.getMaxNumInstancesToCreate() > projectedInstanceCountPerConfig.getOrDefault(config, 0))
                .collect(Collectors.toList());
    ...
    Stream<InstanceOperationTracker.InstanceOperation> filterInstanceOperationsForConfig(
            InstanceConfiguration config, Stream<InstanceOperationTracker.InstanceOperation> instanceOperations) {
